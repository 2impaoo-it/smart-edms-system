...
export function ManagerDashboard({ user, onNavigate }: { user: any, onNavigate: (path: string) => void }) {
        ...
        try {
            ...
            if (Array.isArray(orgRes.data)) {
                const staff = orgRes.data.map((u: any) => ({
                    ...
                    isOnline: Math.random() > 0.5 // Mock online status as backend doesn't track it yet
    ...
    const handleQuickSignConfirm = () => {
        // This would call a bulk sign API if exists, or loop signDocument
    ...
    const pendingFiles = files; // Already filtered by API
        ...
        <div className="space-y-6 lg:space-y-8 animate-in fade-in duration-700 pb-20">
                                    ...
                                    pendingFiles.map(file => (
                                        <div 
                                            ...
                                            className={cn(
                                                ...
                                                selectedForQuickSign.includes(String(file.id)) ? "border-primary bg-primary/5" : "border-transparent bg-slate-50 hover:bg-slate-100"
                                            )}
                                        >
                                            <div className={cn(
                                            ...
                                            <div>
                                                <h4 className="font-bold text-sm">{file.name || file.originalName}</h4>
                        ...
                        >
                            <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-slate-50/50 shrink-0">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-black text-lg">{fileToView.name || fileToView.originalName}</h3>
                            ...
                            <div className="flex-1 bg-slate-200/50 flex items-center justify-center p-8 overflow-auto">
                                {/* Using real preview if possible, otherwise placeholder */}
                                <iframe 
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground font-medium">Chào sếp <span className="text-primary font-black uppercase">{safeUser.name || safeUser.fullName || safeUser.username}</span>. Bạn có tài liệu mới.</p>
                </div>
                        ...
                        filteredFiles.map(file => (
                            <div key={file.id} className="glass-panel p-6 rounded-[32px] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 group hover:border-primary/40 transition-all shadow-xl bg-white/40">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-base">{file.name || file.originalName}</h4>
                                        <div className="flex items-center gap-3 mt-1 opacity-60">
                                            <span className="text-[10px] font-bold text-muted-foreground uppercase">User ID: {file.userId || file.ownerId}</span>
                                            <div className="w-1 h-1 rounded-full bg-slate-300" />
                                            <span className="text-[10px] font-bold text-muted-foreground uppercase">{new Date(file.updatedAt || file.createdAt).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div key={file.id} className="glass-panel p-5 rounded-[24px] border-l-4 border-l-success shadow-lg bg-white/60 transition-transform hover:translate-x-1">
                            <div className="flex justify-between items-start mb-3">
                                <div className="flex items-start gap-3 text-success">
                                    ...
                                    <span className="text-xs font-bold line-clamp-2 leading-tight">{file.name || file.originalName}</span>
                                </div>
                            </div>
                            <div className="flex gap-2 mt-3 pt-3 border-t border-slate-200/50">
                                <button onClick={() => setViewFileId(String(file.id))} className="flex-1 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-[9px] font-black uppercase transition-colors flex items-center justify-center gap-1.5">
                                    <Eye className="w-3 h-3" /> Xem
                                </button>
                                <button onClick={() => handleDownload(file)} className="p-2 rounded-lg bg-slate-100 hover:bg-primary hover:text-white text-primary transition-colors">
